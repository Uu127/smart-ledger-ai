...
export function Layout({ auth, children }: LayoutProps) {
  ...
  return (
    // 🌟 select-none を追加して長押し選択を禁止！完全にアプリの手触りになります
          ...
          {navItems.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`relative flex flex-col items-center gap-1.5 px-8 py-2 transition-all duration-300 active:scale-95 ${

...
export function LedgerList() {
  ...
  return (
    <motion.div 
      ...
      className="p-4 space-y-6 pb-24" // 下のナビゲーションバーに隠れないようにpb-24を設定
    >
      <header className="flex items-end justify-between px-2">
        ...
        <div className="flex gap-2">
          <button onClick={refresh} className="p-2.5 rounded-full bg-white border border-slate-100 text-slate-400 active:rotate-180 transition-all">
            <RefreshCw className="w-5 h-5" />
          </button>
          <button
            onClick={() => downloadCsv(entries)}
            disabled={entries.length === 0}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary text-white text-xs font-bold shadow-lg shadow-slate-200 disabled:opacity-20 transition-all active:scale-95"
          >
      ...
      {sorted.length === 0 ? (
        ...
        <div className="space-y-3">
          {sorted.map((e) => (
            <div key={e.id} className="bg-white p-5 rounded-[1.5rem] shadow-sm border border-slate-50 flex items-center justify-between group active:bg-slate-50 transition-all">

...
import {
  ...
  Sparkles, AlertCircle, X, Plus, Package
} from "lucide-react";
...
export function ReceiptInput() {
  ...
  const [showAssetForm, setShowAssetForm] = useState(false); // 固定資産フォームの表示
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setSubmitted(false); setShowAssetForm(false);
  ...
  // 10万円以上は固定資産の可能性を示唆
  const isLargeAmount = amountNum >= 100000;
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
                <h3 className="font-black text-slate-800 flex items-center gap-2">
                  {loading
                    ...
                    : showAssetForm ? "固定資産として登録" : "経費の内容を確認"
                  }
                </h3>
                ...
                <div className="px-6 pt-5 pb-12">
                  ...
                  {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      ...
                      {/* 10万円以上の場合：固定資産の提案バナー */}
                      {isLargeAmount && !submitted && (
                        <motion.div
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className="text-xs font-black text-orange-700">10万円以上の購入は固定資産の可能性があります</p>
                            <p className="text-[10px] font-bold text-orange-500 mt-0.5">
                              パソコン・車・設備等は減価償却で複数年に分けて経費計上できます
                            </p>
